import{d as YA,B as KA,aa as lA,c as OA,o as u,m as j,e as x,w as Z,f as U,ew as qA,cC as PA,z as _A,q as _,n as S,b as $A,H as f,O as $,G as AB,F as TA,D as BB,aT as nB,ad as EB,ae as MB}from"./index-bd50fbe2.js";import{_ as QB}from"./c-key-value-list.vue_vue_type_script_setup_true_lang-e3deca2e.js";import{T as IB}from"./TextareaCopyable-307e7842.js";import{l as K}from"./index-5483f9e5.js";import{b as wB}from"./index-50a5cdf9.js";import{i as DB}from"./index-44f7831e.js";import{r as gB}from"./empty-6444a79b.js";import{f as CB}from"./convert-3a925b00.js";import"./base64-11d7ac16.js";import"./index-a6f01dde.js";import"./downloadBase64-2afed79b.js";import"./index-59cddc9a.js";import"./index-dbbd6507.js";import"./Copy-e5e09d26.js";import"./Scrollbar-bc5b5dc5.js";import"./index-2fd81833.js";import"./string_decoder-48e14a0c.js";import"./inherits_browser-db1b152f.js";import"./index-d5b34180.js";import"./index-a169f0b1.js";import"./hash-36303d7c.js";import"./empty-c5f13335.js";const NB={flex:"","flex-col":"","gap-2":""},GB=YA({__name:"pdf-signature-details",props:{signature:{}},setup(D){const g=D,{signature:C}=KA(g),{t:N}=lA(),T={validityPeriod:N("tools.pdf-signature-details.text.validity-period"),issuedBy:N("tools.pdf-signature-details.text.issued-by"),issuedTo:N("tools.pdf-signature-details.text.issued-to"),pemCertificate:N("tools.pdf-signature-details.text.pem-certificate")},r=OA(()=>C.value.meta.certs.map((G,Q)=>({...G,validityPeriod:{notBefore:new Date(G.validityPeriod.notBefore).toLocaleString(),notAfter:new Date(G.validityPeriod.notAfter).toLocaleString()},certificateName:N("tools.pdf-signature-details.text.certificate-index-1",[Q+1])})));return(G,Q)=>{const e=QB,a=qA,s=PA;return u(),j("div",NB,[x(s,{data:U(r),headers:T},{validityPeriod:Z(({value:t})=>[x(e,{items:[{label:U(N)("tools.pdf-signature-details.text.not-before"),value:t.notBefore},{label:U(N)("tools.pdf-signature-details.text.not-after"),value:t.notAfter}]},null,8,["items"])]),issuedBy:Z(({value:t})=>[x(e,{items:[{label:U(N)("tools.pdf-signature-details.text.common-name"),value:t.commonName},{label:U(N)("tools.pdf-signature-details.text.organization-name"),value:t.organizationName},{label:U(N)("tools.pdf-signature-details.text.country-name"),value:t.countryName},{label:U(N)("tools.pdf-signature-details.text.locality-name"),value:t.localityName},{label:U(N)("tools.pdf-signature-details.text.organizational-unit-name"),value:t.organizationalUnitName},{label:U(N)("tools.pdf-signature-details.text.state-or-province-name"),value:t.stateOrProvinceName}]},null,8,["items"])]),issuedTo:Z(({value:t})=>[x(e,{items:[{label:U(N)("tools.pdf-signature-details.text.common-name"),value:t.commonName},{label:U(N)("tools.pdf-signature-details.text.organization-name"),value:t.organizationName},{label:U(N)("tools.pdf-signature-details.text.country-name"),value:t.countryName},{label:U(N)("tools.pdf-signature-details.text.locality-name"),value:t.localityName},{label:U(N)("tools.pdf-signature-details.text.organizational-unit-name"),value:t.organizationalUnitName},{label:U(N)("tools.pdf-signature-details.text.state-or-province-name"),value:t.stateOrProvinceName}]},null,8,["items"])]),pemCertificate:Z(({value:t})=>[x(a,{value:t,label:U(N)("tools.pdf-signature-details.text.view-pem-cert")},{value:Z(()=>[x(IB,{"text-xs":"",value:t,"copy-placement":"none"},null,8,["value"])]),_:2},1032,["value","label"])]),_:1},8,["data"])])}}}),RA="TYPE_UNKNOWN",VB="TYPE_INPUT",TB="TYPE_PARSE",tB="TYPE_BYTE_RANGE",FB="VERIFY_SIGNATURE",UB="UNSUPPORTED_SUBFILTER";let eA=class extends Error{constructor(g,C=RA){super(g),this.type=C}};Object.assign(eA,{TYPE_UNKNOWN:RA,TYPE_INPUT:VB,TYPE_PARSE:TB,TYPE_BYTE_RANGE:tB,VERIFY_SIGNATURE:FB,UNSUPPORTED_SUBFILTER:UB});var BA=eA,nA={};(function(D){var g=wB,C=DB,N=typeof Symbol=="function"&&typeof Symbol.for=="function"?Symbol.for("nodejs.util.inspect.custom"):null;D.Buffer=Q,D.SlowBuffer=MA,D.INSPECT_MAX_BYTES=50;var T=2147483647;D.kMaxLength=T,Q.TYPED_ARRAY_SUPPORT=r(),!Q.TYPED_ARRAY_SUPPORT&&typeof console<"u"&&typeof console.error=="function"&&console.error("This browser lacks typed array (Uint8Array) support which is required by `buffer` v5.x. Use `buffer` v4.x if you require old browser support.");function r(){try{var n=new Uint8Array(1),A={foo:function(){return 42}};return Object.setPrototypeOf(A,Uint8Array.prototype),Object.setPrototypeOf(n,A),n.foo()===42}catch{return!1}}Object.defineProperty(Q.prototype,"parent",{enumerable:!0,get:function(){if(Q.isBuffer(this))return this.buffer}}),Object.defineProperty(Q.prototype,"offset",{enumerable:!0,get:function(){if(Q.isBuffer(this))return this.byteOffset}});function G(n){if(n>T)throw new RangeError('The value "'+n+'" is invalid for option "size"');var A=new Uint8Array(n);return Object.setPrototypeOf(A,Q.prototype),A}function Q(n,A,B){if(typeof n=="number"){if(typeof A=="string")throw new TypeError('The "string" argument must be of type string. Received type number');return t(n)}return e(n,A,B)}Q.poolSize=8192;function e(n,A,B){if(typeof n=="string")return z(n,A);if(ArrayBuffer.isView(n))return b(n);if(n==null)throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+typeof n);if(d(n,ArrayBuffer)||n&&d(n.buffer,ArrayBuffer)||typeof SharedArrayBuffer<"u"&&(d(n,SharedArrayBuffer)||n&&d(n.buffer,SharedArrayBuffer)))return k(n,A,B);if(typeof n=="number")throw new TypeError('The "value" argument must not be of type number. Received type number');var E=n.valueOf&&n.valueOf();if(E!=null&&E!==n)return Q.from(E,A,B);var M=EA(n);if(M)return M;if(typeof Symbol<"u"&&Symbol.toPrimitive!=null&&typeof n[Symbol.toPrimitive]=="function")return Q.from(n[Symbol.toPrimitive]("string"),A,B);throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+typeof n)}Q.from=function(n,A,B){return e(n,A,B)},Object.setPrototypeOf(Q.prototype,Uint8Array.prototype),Object.setPrototypeOf(Q,Uint8Array);function a(n){if(typeof n!="number")throw new TypeError('"size" argument must be of type number');if(n<0)throw new RangeError('The value "'+n+'" is invalid for option "size"')}function s(n,A,B){return a(n),n<=0?G(n):A!==void 0?typeof B=="string"?G(n).fill(A,B):G(n).fill(A):G(n)}Q.alloc=function(n,A,B){return s(n,A,B)};function t(n){return a(n),G(n<0?0:m(n)|0)}Q.allocUnsafe=function(n){return t(n)},Q.allocUnsafeSlow=function(n){return t(n)};function z(n,A){if((typeof A!="string"||A==="")&&(A="utf8"),!Q.isEncoding(A))throw new TypeError("Unknown encoding: "+A);var B=H(n,A)|0,E=G(B),M=E.write(n,A);return M!==B&&(E=E.slice(0,M)),E}function b(n){for(var A=n.length<0?0:m(n.length)|0,B=G(A),E=0;E<A;E+=1)B[E]=n[E]&255;return B}function k(n,A,B){if(A<0||n.byteLength<A)throw new RangeError('"offset" is outside of buffer bounds');if(n.byteLength<A+(B||0))throw new RangeError('"length" is outside of buffer bounds');var E;return A===void 0&&B===void 0?E=new Uint8Array(n):B===void 0?E=new Uint8Array(n,A):E=new Uint8Array(n,A,B),Object.setPrototypeOf(E,Q.prototype),E}function EA(n){if(Q.isBuffer(n)){var A=m(n.length)|0,B=G(A);return B.length===0||n.copy(B,0,0,A),B}if(n.length!==void 0)return typeof n.length!="number"||P(n.length)?G(0):b(n);if(n.type==="Buffer"&&Array.isArray(n.data))return b(n.data)}function m(n){if(n>=T)throw new RangeError("Attempt to allocate Buffer larger than maximum size: 0x"+T.toString(16)+" bytes");return n|0}function MA(n){return+n!=n&&(n=0),Q.alloc(+n)}Q.isBuffer=function(A){return A!=null&&A._isBuffer===!0&&A!==Q.prototype},Q.compare=function(A,B){if(d(A,Uint8Array)&&(A=Q.from(A,A.offset,A.byteLength)),d(B,Uint8Array)&&(B=Q.from(B,B.offset,B.byteLength)),!Q.isBuffer(A)||!Q.isBuffer(B))throw new TypeError('The "buf1", "buf2" arguments must be one of type Buffer or Uint8Array');if(A===B)return 0;for(var E=A.length,M=B.length,I=0,w=Math.min(E,M);I<w;++I)if(A[I]!==B[I]){E=A[I],M=B[I];break}return E<M?-1:M<E?1:0},Q.isEncoding=function(A){switch(String(A).toLowerCase()){case"hex":case"utf8":case"utf-8":case"ascii":case"latin1":case"binary":case"base64":case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return!0;default:return!1}},Q.concat=function(A,B){if(!Array.isArray(A))throw new TypeError('"list" argument must be an Array of Buffers');if(A.length===0)return Q.alloc(0);var E;if(B===void 0)for(B=0,E=0;E<A.length;++E)B+=A[E].length;var M=Q.allocUnsafe(B),I=0;for(E=0;E<A.length;++E){var w=A[E];if(d(w,Uint8Array)&&(w=Q.from(w)),!Q.isBuffer(w))throw new TypeError('"list" argument must be an Array of Buffers');w.copy(M,I),I+=w.length}return M};function H(n,A){if(Q.isBuffer(n))return n.length;if(ArrayBuffer.isView(n)||d(n,ArrayBuffer))return n.byteLength;if(typeof n!="string")throw new TypeError('The "string" argument must be one of type string, Buffer, or ArrayBuffer. Received type '+typeof n);var B=n.length,E=arguments.length>2&&arguments[2]===!0;if(!E&&B===0)return 0;for(var M=!1;;)switch(A){case"ascii":case"latin1":case"binary":return B;case"utf8":case"utf-8":return q(n).length;case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return B*2;case"hex":return B>>>1;case"base64":return NA(n).length;default:if(M)return E?-1:q(n).length;A=(""+A).toLowerCase(),M=!0}}Q.byteLength=H;function J(n,A,B){var E=!1;if((A===void 0||A<0)&&(A=0),A>this.length||((B===void 0||B>this.length)&&(B=this.length),B<=0)||(B>>>=0,A>>>=0,B<=A))return"";for(n||(n="utf8");;)switch(n){case"hex":return WA(this,A,B);case"utf8":case"utf-8":return IA(this,A,B);case"ascii":return JA(this,A,B);case"latin1":case"binary":return pA(this,A,B);case"base64":return mA(this,A,B);case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return SA(this,A,B);default:if(E)throw new TypeError("Unknown encoding: "+n);n=(n+"").toLowerCase(),E=!0}}Q.prototype._isBuffer=!0;function o(n,A,B){var E=n[A];n[A]=n[B],n[B]=E}Q.prototype.swap16=function(){var A=this.length;if(A%2!==0)throw new RangeError("Buffer size must be a multiple of 16-bits");for(var B=0;B<A;B+=2)o(this,B,B+1);return this},Q.prototype.swap32=function(){var A=this.length;if(A%4!==0)throw new RangeError("Buffer size must be a multiple of 32-bits");for(var B=0;B<A;B+=4)o(this,B,B+3),o(this,B+1,B+2);return this},Q.prototype.swap64=function(){var A=this.length;if(A%8!==0)throw new RangeError("Buffer size must be a multiple of 64-bits");for(var B=0;B<A;B+=8)o(this,B,B+7),o(this,B+1,B+6),o(this,B+2,B+5),o(this,B+3,B+4);return this},Q.prototype.toString=function(){var A=this.length;return A===0?"":arguments.length===0?IA(this,0,A):J.apply(this,arguments)},Q.prototype.toLocaleString=Q.prototype.toString,Q.prototype.equals=function(A){if(!Q.isBuffer(A))throw new TypeError("Argument must be a Buffer");return this===A?!0:Q.compare(this,A)===0},Q.prototype.inspect=function(){var A="",B=D.INSPECT_MAX_BYTES;return A=this.toString("hex",0,B).replace(/(.{2})/g,"$1 ").trim(),this.length>B&&(A+=" ... "),"<Buffer "+A+">"},N&&(Q.prototype[N]=Q.prototype.inspect),Q.prototype.compare=function(A,B,E,M,I){if(d(A,Uint8Array)&&(A=Q.from(A,A.offset,A.byteLength)),!Q.isBuffer(A))throw new TypeError('The "target" argument must be one of type Buffer or Uint8Array. Received type '+typeof A);if(B===void 0&&(B=0),E===void 0&&(E=A?A.length:0),M===void 0&&(M=0),I===void 0&&(I=this.length),B<0||E>A.length||M<0||I>this.length)throw new RangeError("out of range index");if(M>=I&&B>=E)return 0;if(M>=I)return-1;if(B>=E)return 1;if(B>>>=0,E>>>=0,M>>>=0,I>>>=0,this===A)return 0;for(var w=I-M,V=E-B,F=Math.min(w,V),c=this.slice(M,I),l=A.slice(B,E),i=0;i<F;++i)if(c[i]!==l[i]){w=c[i],V=l[i];break}return w<V?-1:V<w?1:0};function p(n,A,B,E,M){if(n.length===0)return-1;if(typeof B=="string"?(E=B,B=0):B>2147483647?B=2147483647:B<-2147483648&&(B=-2147483648),B=+B,P(B)&&(B=M?0:n.length-1),B<0&&(B=n.length+B),B>=n.length){if(M)return-1;B=n.length-1}else if(B<0)if(M)B=0;else return-1;if(typeof A=="string"&&(A=Q.from(A,E)),Q.isBuffer(A))return A.length===0?-1:W(n,A,B,E,M);if(typeof A=="number")return A=A&255,typeof Uint8Array.prototype.indexOf=="function"?M?Uint8Array.prototype.indexOf.call(n,A,B):Uint8Array.prototype.lastIndexOf.call(n,A,B):W(n,[A],B,E,M);throw new TypeError("val must be string, number or Buffer")}function W(n,A,B,E,M){var I=1,w=n.length,V=A.length;if(E!==void 0&&(E=String(E).toLowerCase(),E==="ucs2"||E==="ucs-2"||E==="utf16le"||E==="utf-16le")){if(n.length<2||A.length<2)return-1;I=2,w/=2,V/=2,B/=2}function F(GA,VA){return I===1?GA[VA]:GA.readUInt16BE(VA*I)}var c;if(M){var l=-1;for(c=B;c<w;c++)if(F(n,c)===F(A,l===-1?0:c-l)){if(l===-1&&(l=c),c-l+1===V)return l*I}else l!==-1&&(c-=c-l),l=-1}else for(B+V>w&&(B=w-V),c=B;c>=0;c--){for(var i=!0,v=0;v<V;v++)if(F(n,c+v)!==F(A,v)){i=!1;break}if(i)return c}return-1}Q.prototype.includes=function(A,B,E){return this.indexOf(A,B,E)!==-1},Q.prototype.indexOf=function(A,B,E){return p(this,A,B,E,!0)},Q.prototype.lastIndexOf=function(A,B,E){return p(this,A,B,E,!1)};function O(n,A,B,E){B=Number(B)||0;var M=n.length-B;E?(E=Number(E),E>M&&(E=M)):E=M;var I=A.length;E>I/2&&(E=I/2);for(var w=0;w<E;++w){var V=parseInt(A.substr(w*2,2),16);if(P(V))return w;n[B+w]=V}return w}function ZA(n,A,B,E){return X(q(A,n.length-B),n,B,E)}function QA(n,A,B,E){return X(vA(A),n,B,E)}function jA(n,A,B,E){return QA(n,A,B,E)}function yA(n,A,B,E){return X(NA(A),n,B,E)}function zA(n,A,B,E){return X(fA(A,n.length-B),n,B,E)}Q.prototype.write=function(A,B,E,M){if(B===void 0)M="utf8",E=this.length,B=0;else if(E===void 0&&typeof B=="string")M=B,E=this.length,B=0;else if(isFinite(B))B=B>>>0,isFinite(E)?(E=E>>>0,M===void 0&&(M="utf8")):(M=E,E=void 0);else throw new Error("Buffer.write(string, encoding, offset[, length]) is no longer supported");var I=this.length-B;if((E===void 0||E>I)&&(E=I),A.length>0&&(E<0||B<0)||B>this.length)throw new RangeError("Attempt to write outside buffer bounds");M||(M="utf8");for(var w=!1;;)switch(M){case"hex":return O(this,A,B,E);case"utf8":case"utf-8":return ZA(this,A,B,E);case"ascii":return QA(this,A,B,E);case"latin1":case"binary":return jA(this,A,B,E);case"base64":return yA(this,A,B,E);case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return zA(this,A,B,E);default:if(w)throw new TypeError("Unknown encoding: "+M);M=(""+M).toLowerCase(),w=!0}},Q.prototype.toJSON=function(){return{type:"Buffer",data:Array.prototype.slice.call(this._arr||this,0)}};function mA(n,A,B){return A===0&&B===n.length?g.fromByteArray(n):g.fromByteArray(n.slice(A,B))}function IA(n,A,B){B=Math.min(n.length,B);for(var E=[],M=A;M<B;){var I=n[M],w=null,V=I>239?4:I>223?3:I>191?2:1;if(M+V<=B){var F,c,l,i;switch(V){case 1:I<128&&(w=I);break;case 2:F=n[M+1],(F&192)===128&&(i=(I&31)<<6|F&63,i>127&&(w=i));break;case 3:F=n[M+1],c=n[M+2],(F&192)===128&&(c&192)===128&&(i=(I&15)<<12|(F&63)<<6|c&63,i>2047&&(i<55296||i>57343)&&(w=i));break;case 4:F=n[M+1],c=n[M+2],l=n[M+3],(F&192)===128&&(c&192)===128&&(l&192)===128&&(i=(I&15)<<18|(F&63)<<12|(c&63)<<6|l&63,i>65535&&i<1114112&&(w=i))}}w===null?(w=65533,V=1):w>65535&&(w-=65536,E.push(w>>>10&1023|55296),w=56320|w&1023),E.push(w),M+=V}return HA(E)}var wA=4096;function HA(n){var A=n.length;if(A<=wA)return String.fromCharCode.apply(String,n);for(var B="",E=0;E<A;)B+=String.fromCharCode.apply(String,n.slice(E,E+=wA));return B}function JA(n,A,B){var E="";B=Math.min(n.length,B);for(var M=A;M<B;++M)E+=String.fromCharCode(n[M]&127);return E}function pA(n,A,B){var E="";B=Math.min(n.length,B);for(var M=A;M<B;++M)E+=String.fromCharCode(n[M]);return E}function WA(n,A,B){var E=n.length;(!A||A<0)&&(A=0),(!B||B<0||B>E)&&(B=E);for(var M="",I=A;I<B;++I)M+=LA[n[I]];return M}function SA(n,A,B){for(var E=n.slice(A,B),M="",I=0;I<E.length;I+=2)M+=String.fromCharCode(E[I]+E[I+1]*256);return M}Q.prototype.slice=function(A,B){var E=this.length;A=~~A,B=B===void 0?E:~~B,A<0?(A+=E,A<0&&(A=0)):A>E&&(A=E),B<0?(B+=E,B<0&&(B=0)):B>E&&(B=E),B<A&&(B=A);var M=this.subarray(A,B);return Object.setPrototypeOf(M,Q.prototype),M};function Y(n,A,B){if(n%1!==0||n<0)throw new RangeError("offset is not uint");if(n+A>B)throw new RangeError("Trying to access beyond buffer length")}Q.prototype.readUIntLE=function(A,B,E){A=A>>>0,B=B>>>0,E||Y(A,B,this.length);for(var M=this[A],I=1,w=0;++w<B&&(I*=256);)M+=this[A+w]*I;return M},Q.prototype.readUIntBE=function(A,B,E){A=A>>>0,B=B>>>0,E||Y(A,B,this.length);for(var M=this[A+--B],I=1;B>0&&(I*=256);)M+=this[A+--B]*I;return M},Q.prototype.readUInt8=function(A,B){return A=A>>>0,B||Y(A,1,this.length),this[A]},Q.prototype.readUInt16LE=function(A,B){return A=A>>>0,B||Y(A,2,this.length),this[A]|this[A+1]<<8},Q.prototype.readUInt16BE=function(A,B){return A=A>>>0,B||Y(A,2,this.length),this[A]<<8|this[A+1]},Q.prototype.readUInt32LE=function(A,B){return A=A>>>0,B||Y(A,4,this.length),(this[A]|this[A+1]<<8|this[A+2]<<16)+this[A+3]*16777216},Q.prototype.readUInt32BE=function(A,B){return A=A>>>0,B||Y(A,4,this.length),this[A]*16777216+(this[A+1]<<16|this[A+2]<<8|this[A+3])},Q.prototype.readIntLE=function(A,B,E){A=A>>>0,B=B>>>0,E||Y(A,B,this.length);for(var M=this[A],I=1,w=0;++w<B&&(I*=256);)M+=this[A+w]*I;return I*=128,M>=I&&(M-=Math.pow(2,8*B)),M},Q.prototype.readIntBE=function(A,B,E){A=A>>>0,B=B>>>0,E||Y(A,B,this.length);for(var M=B,I=1,w=this[A+--M];M>0&&(I*=256);)w+=this[A+--M]*I;return I*=128,w>=I&&(w-=Math.pow(2,8*B)),w},Q.prototype.readInt8=function(A,B){return A=A>>>0,B||Y(A,1,this.length),this[A]&128?(255-this[A]+1)*-1:this[A]},Q.prototype.readInt16LE=function(A,B){A=A>>>0,B||Y(A,2,this.length);var E=this[A]|this[A+1]<<8;return E&32768?E|4294901760:E},Q.prototype.readInt16BE=function(A,B){A=A>>>0,B||Y(A,2,this.length);var E=this[A+1]|this[A]<<8;return E&32768?E|4294901760:E},Q.prototype.readInt32LE=function(A,B){return A=A>>>0,B||Y(A,4,this.length),this[A]|this[A+1]<<8|this[A+2]<<16|this[A+3]<<24},Q.prototype.readInt32BE=function(A,B){return A=A>>>0,B||Y(A,4,this.length),this[A]<<24|this[A+1]<<16|this[A+2]<<8|this[A+3]},Q.prototype.readFloatLE=function(A,B){return A=A>>>0,B||Y(A,4,this.length),C.read(this,A,!0,23,4)},Q.prototype.readFloatBE=function(A,B){return A=A>>>0,B||Y(A,4,this.length),C.read(this,A,!1,23,4)},Q.prototype.readDoubleLE=function(A,B){return A=A>>>0,B||Y(A,8,this.length),C.read(this,A,!0,52,8)},Q.prototype.readDoubleBE=function(A,B){return A=A>>>0,B||Y(A,8,this.length),C.read(this,A,!1,52,8)};function R(n,A,B,E,M,I){if(!Q.isBuffer(n))throw new TypeError('"buffer" argument must be a Buffer instance');if(A>M||A<I)throw new RangeError('"value" argument is out of bounds');if(B+E>n.length)throw new RangeError("Index out of range")}Q.prototype.writeUIntLE=function(A,B,E,M){if(A=+A,B=B>>>0,E=E>>>0,!M){var I=Math.pow(2,8*E)-1;R(this,A,B,E,I,0)}var w=1,V=0;for(this[B]=A&255;++V<E&&(w*=256);)this[B+V]=A/w&255;return B+E},Q.prototype.writeUIntBE=function(A,B,E,M){if(A=+A,B=B>>>0,E=E>>>0,!M){var I=Math.pow(2,8*E)-1;R(this,A,B,E,I,0)}var w=E-1,V=1;for(this[B+w]=A&255;--w>=0&&(V*=256);)this[B+w]=A/V&255;return B+E},Q.prototype.writeUInt8=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,1,255,0),this[B]=A&255,B+1},Q.prototype.writeUInt16LE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,2,65535,0),this[B]=A&255,this[B+1]=A>>>8,B+2},Q.prototype.writeUInt16BE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,2,65535,0),this[B]=A>>>8,this[B+1]=A&255,B+2},Q.prototype.writeUInt32LE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,4,4294967295,0),this[B+3]=A>>>24,this[B+2]=A>>>16,this[B+1]=A>>>8,this[B]=A&255,B+4},Q.prototype.writeUInt32BE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,4,4294967295,0),this[B]=A>>>24,this[B+1]=A>>>16,this[B+2]=A>>>8,this[B+3]=A&255,B+4},Q.prototype.writeIntLE=function(A,B,E,M){if(A=+A,B=B>>>0,!M){var I=Math.pow(2,8*E-1);R(this,A,B,E,I-1,-I)}var w=0,V=1,F=0;for(this[B]=A&255;++w<E&&(V*=256);)A<0&&F===0&&this[B+w-1]!==0&&(F=1),this[B+w]=(A/V>>0)-F&255;return B+E},Q.prototype.writeIntBE=function(A,B,E,M){if(A=+A,B=B>>>0,!M){var I=Math.pow(2,8*E-1);R(this,A,B,E,I-1,-I)}var w=E-1,V=1,F=0;for(this[B+w]=A&255;--w>=0&&(V*=256);)A<0&&F===0&&this[B+w+1]!==0&&(F=1),this[B+w]=(A/V>>0)-F&255;return B+E},Q.prototype.writeInt8=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,1,127,-128),A<0&&(A=255+A+1),this[B]=A&255,B+1},Q.prototype.writeInt16LE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,2,32767,-32768),this[B]=A&255,this[B+1]=A>>>8,B+2},Q.prototype.writeInt16BE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,2,32767,-32768),this[B]=A>>>8,this[B+1]=A&255,B+2},Q.prototype.writeInt32LE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,4,2147483647,-2147483648),this[B]=A&255,this[B+1]=A>>>8,this[B+2]=A>>>16,this[B+3]=A>>>24,B+4},Q.prototype.writeInt32BE=function(A,B,E){return A=+A,B=B>>>0,E||R(this,A,B,4,2147483647,-2147483648),A<0&&(A=4294967295+A+1),this[B]=A>>>24,this[B+1]=A>>>16,this[B+2]=A>>>8,this[B+3]=A&255,B+4};function DA(n,A,B,E,M,I){if(B+E>n.length)throw new RangeError("Index out of range");if(B<0)throw new RangeError("Index out of range")}function gA(n,A,B,E,M){return A=+A,B=B>>>0,M||DA(n,A,B,4),C.write(n,A,B,E,23,4),B+4}Q.prototype.writeFloatLE=function(A,B,E){return gA(this,A,B,!0,E)},Q.prototype.writeFloatBE=function(A,B,E){return gA(this,A,B,!1,E)};function CA(n,A,B,E,M){return A=+A,B=B>>>0,M||DA(n,A,B,8),C.write(n,A,B,E,52,8),B+8}Q.prototype.writeDoubleLE=function(A,B,E){return CA(this,A,B,!0,E)},Q.prototype.writeDoubleBE=function(A,B,E){return CA(this,A,B,!1,E)},Q.prototype.copy=function(A,B,E,M){if(!Q.isBuffer(A))throw new TypeError("argument should be a Buffer");if(E||(E=0),!M&&M!==0&&(M=this.length),B>=A.length&&(B=A.length),B||(B=0),M>0&&M<E&&(M=E),M===E||A.length===0||this.length===0)return 0;if(B<0)throw new RangeError("targetStart out of bounds");if(E<0||E>=this.length)throw new RangeError("Index out of range");if(M<0)throw new RangeError("sourceEnd out of bounds");M>this.length&&(M=this.length),A.length-B<M-E&&(M=A.length-B+E);var I=M-E;if(this===A&&typeof Uint8Array.prototype.copyWithin=="function")this.copyWithin(B,E,M);else if(this===A&&E<B&&B<M)for(var w=I-1;w>=0;--w)A[w+B]=this[w+E];else Uint8Array.prototype.set.call(A,this.subarray(E,M),B);return I},Q.prototype.fill=function(A,B,E,M){if(typeof A=="string"){if(typeof B=="string"?(M=B,B=0,E=this.length):typeof E=="string"&&(M=E,E=this.length),M!==void 0&&typeof M!="string")throw new TypeError("encoding must be a string");if(typeof M=="string"&&!Q.isEncoding(M))throw new TypeError("Unknown encoding: "+M);if(A.length===1){var I=A.charCodeAt(0);(M==="utf8"&&I<128||M==="latin1")&&(A=I)}}else typeof A=="number"?A=A&255:typeof A=="boolean"&&(A=Number(A));if(B<0||this.length<B||this.length<E)throw new RangeError("Out of range index");if(E<=B)return this;B=B>>>0,E=E===void 0?this.length:E>>>0,A||(A=0);var w;if(typeof A=="number")for(w=B;w<E;++w)this[w]=A;else{var V=Q.isBuffer(A)?A:Q.from(A,M),F=V.length;if(F===0)throw new TypeError('The value "'+A+'" is invalid for argument "value"');for(w=0;w<E-B;++w)this[w+B]=V[w%F]}return this};var kA=/[^+/0-9A-Za-z-_]/g;function XA(n){if(n=n.split("=")[0],n=n.trim().replace(kA,""),n.length<2)return"";for(;n.length%4!==0;)n=n+"=";return n}function q(n,A){A=A||1/0;for(var B,E=n.length,M=null,I=[],w=0;w<E;++w){if(B=n.charCodeAt(w),B>55295&&B<57344){if(!M){if(B>56319){(A-=3)>-1&&I.push(239,191,189);continue}else if(w+1===E){(A-=3)>-1&&I.push(239,191,189);continue}M=B;continue}if(B<56320){(A-=3)>-1&&I.push(239,191,189),M=B;continue}B=(M-55296<<10|B-56320)+65536}else M&&(A-=3)>-1&&I.push(239,191,189);if(M=null,B<128){if((A-=1)<0)break;I.push(B)}else if(B<2048){if((A-=2)<0)break;I.push(B>>6|192,B&63|128)}else if(B<65536){if((A-=3)<0)break;I.push(B>>12|224,B>>6&63|128,B&63|128)}else if(B<1114112){if((A-=4)<0)break;I.push(B>>18|240,B>>12&63|128,B>>6&63|128,B&63|128)}else throw new Error("Invalid code point")}return I}function vA(n){for(var A=[],B=0;B<n.length;++B)A.push(n.charCodeAt(B)&255);return A}function fA(n,A){for(var B,E,M,I=[],w=0;w<n.length&&!((A-=2)<0);++w)B=n.charCodeAt(w),E=B>>8,M=B%256,I.push(M),I.push(E);return I}function NA(n){return g.toByteArray(XA(n))}function X(n,A,B,E){for(var M=0;M<E&&!(M+B>=A.length||M>=n.length);++M)A[M+B]=n[M];return M}function d(n,A){return n instanceof A||n!=null&&n.constructor!=null&&n.constructor.name!=null&&n.constructor.name===A.name}function P(n){return n!==n}var LA=function(){for(var n="0123456789abcdef",A=new Array(256),B=0;B<16;++B)for(var E=B*16,M=0;M<16;++M)A[E+M]=n[B]+n[M];return A}()})(nA);const tA=K,{Buffer:AA}=nA,y=BA,rB=D=>{try{return AA.isBuffer(D)?D:AA.from(D)}catch{throw new y("PDF expected as Buffer.",y.TYPE_INPUT)}},cB=D=>{const g=D.toString().match(/\/SubFilter\s*\/([\w.]*)/),C=Array.isArray(g)&&g[1];if(!C)throw new y("cannot find subfilter",y.TYPE_PARSE);if(!["adbe.pkcs7.detached","etsi.cades.detached"].includes(C.trim().toLowerCase()))throw new y(`subFilter ${C} not supported`,y.UNSUPPORTED_SUBFILTER)},iB=D=>{const g=tA.asn1.fromDer(D,{parseAllBytes:!1});return tA.pkcs7.messageFromAsn1(g)},L=D=>g=>{const C=new RegExp(`/${D}\\s*\\(([\\w.\\s@,]*)`,"g"),N=[...g.matchAll(C)];return N.length?N[N.length-1][1]:null},YB=D=>{const g=AA.isBuffer(D)?D.toString():D;return{reason:L("Reason")(g),contactInfo:L("ContactInfo")(g),location:L("Location")(g),name:L("Name")(g)}};var hA={checkForSubFilter:cB,getSignatureMeta:YB,getMessageFromSignature:iB,preparePDF:rB};const{Buffer:FA}=nA,UA=BA,{getSignatureMeta:lB,preparePDF:RB}=hA,eB="**********",hB=D=>{const g=D.toString().match(/\/ByteRange\s*\[{1}\s*(?:(?:\d*|\/\*{10})\s+){3}(?:\d+|\/\*{10}){1}\s*\]{1}/g);if(!g)throw new UA("Failed to locate ByteRange.",UA.TYPE_PARSE);const C=g.find(r=>r.includes(`/${eB}`)),T=g.map(r=>r.match(/[^[\s]*(?:\d|\/\*{10})/g)).map(r=>r.map(Number));return{byteRangePlaceholder:C,byteRanges:T}},aB=D=>{const g=RB(D),{byteRanges:C}=hB(g),N=[],T=[];C.forEach(G=>{T.push(FA.concat([g.slice(G[0],G[0]+G[1]),g.slice(G[2],G[2]+G[3])]));const Q=g.slice(G[0]+G[1]+1,G[2]).toString("latin1");N.push(FA.from(Q,"hex").toString("latin1"))});const r=T.map(G=>lB(G));return{byteRanges:C,signatureStr:N,signedData:T,signatureMeta:r}};var oB=aB;const dB=D=>g=>D!==g&&g.issued(D),xB=D=>g=>D.find(dB(g)),sB=D=>g=>!D(g),bB=D=>sB(xB(D)),uB=D=>D.findIndex(bB(D)),ZB=D=>g=>D!==g&&D.issued(g),jB=D=>g=>D.findIndex(ZB(g)),aA=D=>{const g=Array.from(D),C=uB(g),N=g.splice(C,1);for(;g.length;){const T=N[0],r=jB(g)(T);if(r===-1)g.splice(r,1);else{const[G]=g.splice(r,1);N.unshift(G)}}return N},yB=D=>aA(D)[0];var zB={sortCertificateChain:aA,getClientCertificate:yB};const mB=[`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

R9I4LtD+gdwyah617jzV/OeBHRnDJELqYzmp
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

j2A781q0tHuu2guQOHXvgR1m0vdXcDazv/wor3ElhVsT/h5/WrQ8
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

akhT2+zNVVXxxvjpoixMptEmX36vWkzaH6byHCx+rgIW0lbQL1dTR+iS
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

ZdWCEOrCMc0u
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

V3bqxbYoOb8VZRzI9neWagqNdwvYkQsEjgfbKbYK7p2CNTUQ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

W8mw0FfB+j564ZfJ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

5Aq7KkzrCWA5zspi2C5u
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

ThaA63p4ZUWiABqvDA1VZDRIuJK58bRQKfJPIx/abKwfROHdI3hRW8cW
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NmdspZS11My5vWo1ViHe2MPr+8ukYEywVaCge1ey
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

Ecgh4LMRkWXbtKaIOM5V
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

aZVDADlN9u6wWk5JRFRYX0KD
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

9vuD4DPTCF+hnMJbn0bWtsuRBmOiBuczrD6ogRLQy7rQkgu2npaqBA+K
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

LXpUq3DDfSJlgnCW
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WD9f
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

8zsxkVrctQgVrXYlCg17LofiDKYGvCYQbTed7N14jHyAxfDZd0jQ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

SC34MCN32EZLeW32jO06f2ARePTpm67VVMB0gNELQp/B
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NnReJaH1ZgUufzkVqSr7UIuOhWn0
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

/Wi3MOxw+3WKt21gZ7IeyLnp2KhvAotnDU0mV3HaIPzBSlCNsSi6
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

SjnRBUkLp7Y3gaVdjKozXoEofKd9J+sAro03
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

JuVvw9y4AyHqnxbxLFS1
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

QCj8KWefshNPZiTEUxnpHikV7+ZtsH8tZ/3zbBt1RqPlShfppNcL
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

D0tVNEYqi4Y7
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

KrcYPqcZ2Qt9sTdBQrC6YB3y/gkRsPCHe6ed
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

tIsPm6Ik8KaN1nrgS7ZklmOVhMJKzRwuJIczYOXD
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

iC3oheGe7IUIarFsNMkd7EgrO3jtZsSOeWmD3n+M
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WGWVjUTR939+J399roD1B0y2PpxxVJkES/1Y+Zj0
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

3KjVSaLtx9zRSX8XAbjIho9OjIgrqJqpisXRAL34VOKa5Vt8sycX
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

6hN35N6MgSGtBxBHEa2HPQfRdbzP82Z+
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

88Tk5c0NqMuRkrF8hey1FGlmDoLnzc7ILaZRfyHBNVOFBkpdn627G190
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

+SvzZpA3
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

pRtK4dw4LTzcqx+QGtVKnO7RcGzM7vRX+Bi6hG6H
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

BOxonbRJRBD0ckscZOf85muQ3Wl9af0AVqW3rLatt8o+Ae+c
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WyVgrtBIGu4G
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

Kc2Jo5YPSjXnTkLAdc0Hz+Ys63su
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

w5Khchf8R7agCyzxxN5DaAhqXzvwdmP7zAYspsbiDrW5viSP
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

qiAeW2GFZECpkJcNrVPSWh2HagCXZWK0vm9qp/UsQu0yrbYhnr68
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

m8rifOoCWCKR
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

eGADbkpyrqXRfboQnoZsG4q5WTP468SQvvG5
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

KuGUaJPHZnJED4AhMmwlxyOAgwrr
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

wpaYT05KnJe32x+SMsj/D1Fu1uwJ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

0nxpbl/f5Wpl
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

i9fgojaGHAeCOnZT/cKi7e97sIBPWA9LUzm9
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

cu51OyLemGhmW/HGY0dVHLqlCFF1pkgl
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

XS/Dr55FBcOEArf9LAhST4Ldo/DUhgkC
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

UfUk7GRSJFClH9voy8l27OyCbvWFGFPouOOaKaqW04MjyaR7YbPMAuhd
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NlK73nZfdklJrX+9ZSCyycErdhh2n1ax
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

UvToiIMrVCjU8jVStDKDYmlkDJGcn5fqdBb9HxEGmpv0
-----END CERTIFICATE-----`],HB=gB,rA=K,JB=mB,pB=()=>HB.rootCertificates||JB,oA=D=>!!pB().find(g=>{try{const C=rA.pki.certificateFromPem(g);return rA.pki.certificateToPem(D)===g||C.issued(D)}catch{return!1}}),dA=D=>!!D.find((g,C)=>D[C+1]&&D[C+1].issued(g)),WB=D=>!!D.find(({validity:{notAfter:g,notBefore:C}})=>g.getTime()<Date.now()||C.getTime()>Date.now()),SB=D=>dA(D)&&oA(D[D.length-1]);var kB={authenticateSignature:SB,verifyCaBundle:dA,verifyRootCert:oA,isCertsExpired:WB};const XB=oB,vB=zB,fB=hA,LB=kB;var xA={extractSignature:XB,...vB,...fB,...LB};const KB=K,{extractSignature:OB,getMessageFromSignature:qB,preparePDF:PB}=xA,cA=D=>D.reduce((g,{name:C,value:N})=>(C&&(g[C]=N),g),{}),_B=D=>{const{issuer:g,subject:C,validity:N}=D;return{issuedBy:cA(g.attributes),issuedTo:cA(C.attributes),validityPeriod:N,pemCertificate:KB.pki.certificateToPem(D)}},sA=D=>D.map(_B).map((g,C)=>C?g:{clientCertificate:!0,...g}),$B=D=>{const g=PB(D),{signatureStr:C}=OB(g);return C.map(N=>{const{certificates:T}=qB(N);return sA(T)})};var bA={extractCertificatesDetails:sA,getCertificatesInfoFromPDF:$B};const h=K,iA=BA,{extractSignature:An,getMessageFromSignature:Bn,getClientCertificate:nn,checkForSubFilter:En,preparePDF:Mn,authenticateSignature:Qn,sortCertificateChain:In,isCertsExpired:wn}=xA,{extractCertificatesDetails:Dn}=bA,gn=(D,g,C)=>{const N=Bn(D),{certificates:T,rawCapture:{signature:r,authenticatedAttributes:G,digestAlgorithm:Q}}=N,e=h.asn1.derToOid(Q),a=h.pki.oids[e].toLowerCase(),s=h.asn1.create(h.asn1.Class.UNIVERSAL,h.asn1.Type.SET,!0,G),t=nn(T),z=h.md[a].create().update(h.asn1.toDer(s).data).digest().getBytes();if(!t.publicKey.verify(z,r))throw new iA("Wrong authenticated attributes",iA.VERIFY_SIGNATURE);const k=h.pki.oids.messageDigest,m=G.find(O=>h.asn1.derToOid(O.value[0].value)===k).value[1].value[0].value,H=h.md[a].create().update(g.toString("latin1")).digest().getBytes()===m,J=In(T),o=Dn(J),p=Qn(J),W=wn(J);return{verified:H&&p&&!W,authenticity:p,integrity:H,expired:W,meta:{certs:o,signatureMeta:C}}};var Cn=D=>{const g=Mn(D);En(g);try{const{signatureStr:C,signedData:N,signatureMeta:T}=An(g),r=N.map((G,Q)=>gn(C[Q],G,T[Q]));return{verified:r.every(G=>G.verified===!0),authenticity:r.every(G=>G.authenticity===!0),integrity:r.every(G=>G.integrity===!0),expired:r.some(G=>G.expired===!0),signatures:r}}catch(C){return{verified:!1,message:C.message,error:C}}};const uA=Cn,{getCertificatesInfoFromPDF:Nn}=bA;Object.assign(uA,{getCertificatesInfoFromPDF:Nn});var Gn=uA;const Vn=_A(Gn),Tn={style:{flex:"0 0 100%"}},tn={"mx-auto":"","max-w-600px":""},Fn={"font-bold":""},Un={key:1},rn={key:0,style:{flex:"0 0 100%"},"mt-5":"",flex:"","flex-col":"","gap-4":""},cn={"mb-2":"","font-bold":""},Xn=YA({__name:"pdf-signature-checker",setup(D){const{t:g}=lA(),C=_([]),N=_("idle"),T=_(null);async function r(G){T.value=G;const Q=await G.arrayBuffer();N.value="loading";try{const{signatures:e}=Vn(Q);C.value=e,N.value="parsed"}catch{C.value=[],N.value="error"}}return(G,Q)=>{const e=nB,a=EB,s=MB,t=GB;return u(),j(TA,null,[S("div",Tn,[S("div",tn,[x(e,{title:U(g)("tools.pdf-signature-checker.texts.title-drag-and-drop-a-pdf-file-here-or-click-to-select-a-file"),accept:".pdf",onFileUpload:r},null,8,["title"]),U(T)?(u(),$A(a,{key:0,"mt-4":"",flex:"","gap-2":""},{default:Z(()=>[S("div",Fn,f(U(T).name),1),S("div",null,f(U(CB)(U(T).size)),1)]),_:1})):$("",!0),U(N)==="error"?(u(),j("div",Un,[x(s,{"mt-4":""},{default:Z(()=>[AB(f(U(g)("tools.pdf-signature-checker.texts.tag-no-signatures-found-in-the-provided-file")),1)]),_:1})])):$("",!0)])]),U(N)==="parsed"&&U(C).length?(u(),j("div",rn,[(u(!0),j(TA,null,BB(U(C),(z,b)=>(u(),j("div",{key:b},[S("div",cn," Signature "+f(b+1)+" certificates : ",1),x(t,{signature:z},null,8,["signature"])]))),128))])):$("",!0)],64)}}});export{Xn as default};
